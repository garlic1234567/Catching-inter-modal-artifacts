...
from .Evaluation import run_evaluation, av1m_run_evaluation
...
def make_optimizer(model, optimizer_config):
    ...
    decay = set()
    no_decay = set()
    whitelist_weight_modules = (torch.nn.Linear, torch.nn.Conv1d, MaskedConv1D, torch.nn.ConvTranspose1d,torch.nn.Conv2d,torch.nn.MaxPool1d)
    blacklist_weight_modules = (LayerNorm, torch.nn.GroupNorm, torch.nn.Embedding, torch.nn.LayerNorm,torch.nn.Parameter,torch.nn.BatchNorm1d)
...
def train_one_epoch(
    ...
    for iter_idx, video_list in enumerate(train_loader, 0):
        ...
        # train_sampler.set_epoch(iter_idx)
...
def av1m_valid_one_epoch(
    ...
    if evaluator is not None:
        ...
        av1m_run_evaluation(results, gt_file,output_file, max_avg_nr_proposal=max_avg_nr_proposal,tiou_thre=tiou_thre,subset=subset,cls_score_file=ext_score_file)   
    else:
        # dump to a pickle file that can be directly used for evaluation
        with open(output_file, "wb") as f:
            pickle.dump(results, f)
