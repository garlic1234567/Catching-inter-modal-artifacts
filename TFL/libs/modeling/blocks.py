...
class GlobalContext(nn.Module):
    def __init__(self, C, embedding_dim):
        super().__init__()
        self.query = nn.Linear(C,embedding_dim)
        self.key = nn.Linear(C,embedding_dim)
        self.value = nn.Linear(C,embedding_dim)
    
    def forward(self, inputs):
        query = self.query(inputs.permute(0,2,1))
        key = self.key(inputs.permute(0,2,1))
        value = self.value(inputs.permute(0,2,1))
        weights = torch.bmm(query.transpose(1,2), key)
        del query,key
        weights = F.softmax(weights, dim=2)
        outputs = torch.bmm(weights, value.transpose(1, 2))
        del weights,value
        return outputs
...
class TemporalMaxer(nn.Module):
    ...
    def forward(self, x, mask, **kwargs):
        ...
        out = self.ds_pooling(x) * out_mask.detach()
        return out, out_mask.bool()


class TemporalUnmaxer(nn.Module):
    def __init__(self, stride):
        super().__init__()
        self.stride = stride

    def forward(self, x):
        # Upsample the input and mask using F.interpolate
        out = F.interpolate(
            x, size = x.size(-1) * self.stride)
        return out
...
class ContractionTwitter(nn.Module):
    def __init__(self,in_channels,out_channels,hidden_dims=256):
        super(ContractionTwitter, self).__init__()
        self.down_1 = DownBlock(in_channel=in_channels, out_channel=hidden_dims)
        self.down_2 = DownBlock(in_channel=hidden_dims, out_channel=out_channels)
...
class ExpansionTwitter(nn.Module):
    def __init__(self,in_channels,out_channels,hidden_dims=2048, tanh=True):
        super(ExpansionTwitter, self).__init__()
        self.up_1 = UpBlock(in_channel=in_channels, out_channel=hidden_dims,output_padding=1)
        self.up_2 = UpBlock(in_channel=hidden_dims, out_channel=out_channels,output_padding=1, last=tanh)
...
class DeepInterpolator(nn.Module):
    def __init__(self, in_channels, out_channels,hidden_channels=512, num_classes=1, norm=True):
    ...
    def forward(self, inputs,mask):
        ...
        return reco, cls_socres   ##inputs.detach(),
    
class DeepInterpolatorTwitter(nn.Module):
    def __init__(self, in_channels, out_channels,hidden_channels=64, num_classes=1, norm=True):
        super(DeepInterpolatorTwitter, self).__init__()
        self.norm = norm
        self.contraction = ContractionTwitter(in_channels,hidden_channels)
        self.expansion = ExpansionTwitter(hidden_channels, out_channels,tanh=False)
        self.conv0 = nn.Sequential(
            nn.Conv1d(hidden_channels, hidden_channels, kernel_size=1, bias=False),
            nn.InstanceNorm1d(hidden_channels),
            nn.LeakyReLU(0.2))
