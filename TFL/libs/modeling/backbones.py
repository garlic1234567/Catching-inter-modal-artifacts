...
from .blocks import (TemporalMaxer,get_sinusoid_encoding, TransformerBlock, MaskedConv1D, MutilModelTransformerBlock,ConvBlock, LayerNorm)
...
class ConvHRLRFullResSelfAttTransformerBackboneRevised(nn.Module):
    ...
    def forward(self, x, norm_x, reco_x, mask):
        ...
        if self.use_abs_pe and (not self.training):
            ...
            norm_x = norm_x + pe[:, :, :T] * mask.to(x.dtype)
            reco_x = reco_x + pe[:, :, :T] * mask.to(x.dtype)
            # fused_inputs = fused_inputs + pe[:, :, :T] * mask.to(x.dtype)
