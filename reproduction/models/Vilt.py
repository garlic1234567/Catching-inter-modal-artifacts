...
class HAMMER(nn.Module):
    ...
    def forward(self, image, inputs, label, fake_image_box, fake_text_pos, alpha=0, is_train=True):
        if is_train:
            ...
            token_label[token_label==0] = -100 # -100 index = padding token  让无效的填充标记不进行cross-attention
            ...
            for batch_idx in range(len(fake_text_pos)):
                ...
                if fake_pos_sample:
                    for pos in fake_pos_sample:
                        token_label[batch_idx, pos] = 1 #把假标记位置的标签设为1
            
            # B = outputs.shape[0]
            # tmg_outputs = outputs.view(B,-1,2)  # 32,384,2,然后变成32，T，2
            ...
            # attention_mask_rm_CLS = inputs.attention_mask[:,1:] # [:,1:] for ingoring class token  32,36
            # active_loss = attention_mask_rm_CLS.reshape(-1) == 1  # 32*36
            # active_logits = tmg_outputs.reshape(-1, 2)  # 512 2
            # active_labels = torch.where(  # active_loss和token_label.view(-1)要相同shape
            #     active_loss, token_label.view(-1), torch.tensor(loss_fct.ignore_index).type_as(token_label)
            # )
            # loss_TMG = loss_fct(active_logits, active_labels)  # active_logits应是1152 2即32 36 2.
