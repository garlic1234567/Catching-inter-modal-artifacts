...
import utils
from dataset import create_dataset, create_sampler, create_loader,create_dataset_vilt,create_dataset_clip
...
def text_input_adjust(text_input, fake_word_pos, device):
    ...
    input_ids_remove_SEP_pad = [x + [0] * (maxlen - len(x)) for x in input_ids_remove_SEP] # only remove SEP as HAMMER is conducted with text with CLS,然后还pad了
    ...
    #text_input.pixel_values = torch.LongTensor(attention_mask_remove_SEP_pad).to(device)
    ...
    for i in range(len(fake_word_pos)): # fake_word_pos 32,50
        ...
        # subword_idx = text_input.word_ids(i) # 把第i句话的字词生成索引。
        length = len(text_input_.input_ids[i]) - 2
        subword_idx_rm_CLSSEP = list(range(length))
        # subword_idx_rm_CLSSEP = subword_idx[1:-1] # 把开始和结束的token去除。
        ...
        for i in fake_word_pos_decimal: 
            fake_token_pos.extend(np.where(subword_idx_rm_CLSSEP_array == i)[0].tolist()) # 如果子词索引是之前标记的fake_word的位置，就返回，
...
def evaluation(args, model, data_loader, tokenizer, device, config):
    ...
    for i, (image, label, text, fake_image_box, fake_word_pos, W, H) in enumerate(metric_logger.log_every(args, data_loader, print_freq, header)):
        ...
        text_input = tokenizer(text,max_length=49, padding='max_length',truncation=True, add_special_tokens=True, return_attention_mask=True, return_token_type_ids=False) # bert的输出是一个list。里面不同长度的id和attention_mask
        ...
        cls_label = torch.ones(len(label), dtype=torch.long).to("cuda:0") 
...
def main_worker(gpu, args, config):
    ...
    #### Model #### 
    tokenizer = CLIPProcessor.from_pretrained("./DGM4/clip-vit-large-patch14")
    if args.log:
